...
export class FiboStructuredPromptBuilder {
     ...
     */
    generateShortDescription(input, layout) {
...
Print-ready quality suitable for sharing on social media or academic presentations.
     ...
     */
    generateObjects(input, layout, colorScheme) {
        const objects = [];
     ...
     */
    generateHeaderObject(title, level, section, colorScheme) {
     ...
     */
    generateConceptObject(concept, index, level, section, colorScheme) {
     ...
     */
    generateConceptVisualDescription(concept, level) {
        if (level === "beginner") {
            return `
...
Show methodology details and technical nuances with annotations.
     ...
     */
    generateConnectorObject(numConcepts, section, colorScheme) {
        return {
     ...
     */
    generateFooterObject(keyFinding, level, section, colorScheme) {
        return {
     ...
     */
    generateTextElements(input, layout, typography) {
        const textElements = [];
     ...
     */
    calculateDynamicFontSize(text, level) {
        const baseSizes = {
            beginner: 18,
            intermediate: 16,
            advanced: 14,
     ...
     */
    generateBackground(colorScheme, level) {
        if (level === "beginner") {
     ...
     */
    generateAesthetics(level, colorScheme) {
        const compositions = {
            beginner:
...
${input.user_preferences?.background ? `The viewer has background in: ${input.user_preferences.background}.` : ""}
     ...
     */
    buildHeaderSection(input, layout) {
        const colorScheme = this.selectColorScheme(input.knowledge_level);
        const headerSection = layout.sections.find((s) => s.content_type === "header");
     ...
     */
    buildConceptSection(concept, index, level, layout) {
     ...
     */
    buildFooterSection(input, layout) {
        const colorScheme = this.selectColorScheme(input.knowledge_level);
        const footerSection = layout.sections.find((s) => s.content_type === "footer");
 ...
 */
export function createFiboPromptBuilder() {
