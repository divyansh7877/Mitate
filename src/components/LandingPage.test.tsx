...
describe('LandingPage', () => {
    ...
    it('validates input', async () => {
        renderWithProvider(<LandingPage />);
        const button = screen.getByRole('button', { name: /generate visual explainer/i });

        fireEvent.click(button);

        await waitFor(() => {
            expect(screen.getByText('Please enter a topic or ArXiv link.')).toBeInTheDocument();
        });
    });
    ...
    it('submits form with valid data', async () => {
        ...
        expect(screen.queryByText('Please enter a topic or ArXiv link.')).not.toBeInTheDocument();
    });
});
